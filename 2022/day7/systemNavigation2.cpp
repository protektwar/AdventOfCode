...
//        std::cout << "file Size in dir: " << dirFileSize << std::endl;
    ...
    systemCurrentPathSize[systemPathLevel-1] += systemCurrentPathSize[systemPathLevel];
    int systemTotalSize = systemCurrentPathSize[systemPathLevel-1];
    int toFreeUp = 30000000 - (70000000 - systemTotalSize);
    std::cout << systemTotalSize << std::endl; 
    std::cout << toFreeUp << std::endl;
    return toFreeUp;
}

int systemUpdate(int toFreeUP)
{
    int systemUpdateFreeUp;
    std::stringstream commandos;
    std::string line, commandName, fileNameDir;
    int isCommand, takeCmdOutput, cmdOutputCounter, fileSize, collectInfo, dirFileSize, totalFileSize;
    int changeDir = 0;
    std::vector<std::string> systemCurrentPath = {};
    std::vector<int> systemUpdateToFreeList;
    int systemCurrentPathSize[100];
    int systemPathLevel = 0;
...
//                    std::cout << commandParts << std::endl;
            ...
            if (changeDir == 1)
            {
                ...
                std::cout <<"fileNameDir added to path -" << fileNameDir<< "-\n";
                if (fileNameDir == ".."){
                    if (systemCurrentPathSize[systemPathLevel] >= toFreeUP){
//                        totalFileSize += systemCurrentPathSize[systemPathLevel];
                        systemUpdateToFreeList.push_back(systemCurrentPathSize[systemPathLevel]);
                        std::cout << "adding to total " << totalFileSize << "\n";
                    }
                    if (systemCurrentPath.back() != "/")
                        systemCurrentPath.pop_back();
...
//        std::cout << "file Size in dir: " << dirFileSize << std::endl;
    ...
    std::sort(systemUpdateToFreeList.begin(), systemUpdateToFreeList.end());
    systemUpdateFreeUp = systemUpdateToFreeList.front(); 
    return systemUpdateFreeUp;
}

int main(void)
{
    int toFreeUp = systemCheck();
    int systemUpdateFreeUp = systemUpdate(toFreeUp);
    std::cout << "What is the total size of that directory? ";
    std::cout << systemUpdateFreeUp << std::endl;
    return 1;
}
