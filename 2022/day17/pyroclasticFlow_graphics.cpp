...
public:
    ...
    void updateFromMatrix(char verticalChamber[N][M])
    {
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                 sf::Vertex* quad = &m_vertices[(i + j * N) * 4];

                // define its 4 corners
                quad[0].position = sf::Vector2f(j * 10, i * 10);
                quad[1].position = sf::Vector2f((j + 1) * 10, i * 10);
                quad[2].position = sf::Vector2f((j + 1) * 10, (i + 1) * 10);
                quad[3].position = sf::Vector2f(j * 10, (i + 1) * 10);
...
//                    E  NE   N  SE   S
    ...
    sf::RenderWindow window(sf::VideoMode(200, 900), "Tetris?!");
    myTetris.init();
    
    while (unitCount != 2022 || window.isOpen()) {
...
//            window.clear(sf::Color::Black);
            myTetris.updateFromMatrix(verticalChamber);
            window.draw(myTetris);
            window.display();            
            usleep(100000);
