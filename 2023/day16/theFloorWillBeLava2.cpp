...
void initEnergizedLayout(std::vector<std::vector<int>> &energizedLayout, int n, int m)
{
  energizedLayout.clear();
  for (int j = 0; j < m; j++){
    energizedLayout.push_back(std::vector<int>(n, 0));
  }
}
...
//  inFile.open("test.txt");
  ...
  for (int i = 0; i < n; i++){
    initEnergizedLayout(energizedLayout, n, m);
    energize(i, 0, n, m, layout, energizedLayout, 0, 1);
    energizedTiles = 0; 
    ...
    initEnergizedLayout(energizedLayout, n, m);
    energize(i, m-1, n, m, layout, energizedLayout, 0, -1);
    energizedTiles = 0; 
    for (auto v: energizedLayout)
      for (auto e: v)
        if (e > 0)
  ...
  for (int j = 0; j < m; j++){
    initEnergizedLayout(energizedLayout, n, m);
    energize(0, j, n, m, layout, energizedLayout, 1, 0);
    energizedTiles = 0; 
    for (auto v: energizedLayout)
      for (auto e: v)
        if (e > 0)
    ...
    if (maxEnergizedTiles < energizedTiles)
      maxEnergizedTiles = energizedTiles;
    initEnergizedLayout(energizedLayout, n, m);
    energize(n-1, j, n, m, layout, energizedLayout, -1, 0);
    energizedTiles = 0; 
    for (auto v: energizedLayout)
      for (auto e: v)
        if (e > 0)
